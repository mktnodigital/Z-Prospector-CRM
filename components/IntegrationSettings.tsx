...
export const IntegrationSettings: React.FC = () => {
  ...
  const handleSaveConfig = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setIntegrations(prev => prev.map(i => i.id === editingItem.id ? { ...i, status: 'CONNECTED' } : i));
    }, 1500);
  };
    ...
    <div className="space-y-10 animate-in pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {integrations.map((item) => (
          <div key={item.id} className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border-2 border-slate-100 dark:border-slate-800 shadow-sm flex flex-col group relative overflow-hidden transition-all hover:border-indigo-500 hover:shadow-2xl">
            <div className={`absolute top-0 right-0 w-32 h-32 opacity-10 blur-3xl -translate-y-10 translate-x-10 ${item.color === 'blue' ? 'bg-blue-500' : item.color === 'indigo' ? 'bg-indigo-500' : 'bg-emerald-500'}`}></div>
            <div className="flex items-start justify-between mb-8 relative z-10">
              <div className={`p-5 rounded-2xl shadow-sm group-hover:scale-110 transition-transform ${getColorClass(item.color)}`}>{getIcon(item.icon)}</div>
              <div className="flex items-center gap-2">
                {item.status === 'CONNECTED' ? (
                  <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest bg-emerald-50 dark:bg-emerald-900/20 px-4 py-1.5 rounded-full border border-emerald-100 dark:border-emerald-800/50">Ativo</span>
                ) : (
                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 dark:bg-slate-800 px-4 py-1.5 rounded-full border border-slate-100 dark:border-slate-800/50">Desconectado</span>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-2 relative z-10">
              ...
              <button 
                ...
                className={`p-4 rounded-xl transition-all shadow-md active:scale-90 ${item.status === 'CONNECTED' ? 'bg-indigo-600 text-white shadow-indigo-100/50' : 'bg-slate-200 dark:bg-slate-800 text-slate-500'}`}
              >
                <Power size={18} />
              </button>
            </div>
      ...
      {showConfigModal && editingItem && (
        <div className="fixed inset-0 z-[310] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-md animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[3.5rem] shadow-2xl p-12 border border-slate-200 dark:border-slate-800 relative">
             <button onClick={() => setShowConfigModal(false)} className="absolute top-10 right-10 p-3 bg-slate-100 dark:bg-slate-800 rounded-2xl text-slate-400 hover:text-rose-500 transition-all"><X size={24} /></button>
             <div className="flex items-center gap-5 mb-10">
                <div className={`p-5 rounded-2xl shadow-sm ${getColorClass(editingItem.color)}`}>{getIcon(editingItem.icon)}</div>
                <div>
                   <h3 className="text-2xl font-black italic uppercase tracking-tight">Setup: {editingItem.name}</h3>
             ...
             <form onSubmit={handleSaveConfig} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="space-y-2">
                      <label className="text-[9px] font-black uppercase text-slate-400 px-4 tracking-widest">Public Key / Client ID</label>
                   ...
                   <div className="space-y-2">
                      <label className="text-[9px] font-black uppercase text-slate-400 px-4 tracking-widest">Access Token / Secret</label>
                ...
                <div className="p-6 bg-indigo-50 dark:bg-indigo-900/20 rounded-3xl border border-indigo-100 dark:border-indigo-800/50 flex gap-4">
                   ...
                   <p className="text-[9px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest leading-relaxed">
                      Criptografia TLS 1.3 ativa para comunicação com gateways. Suas chaves são armazenadas em ambiente isolado (Vault).
                   </p>
                </div>

                <button type="submit" disabled={isProcessingId === editingItem.id} className="w-full py-6 bg-indigo-600 text-white font-black rounded-3xl shadow-xl hover:bg-indigo-700 transition-all uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-3">
                   {isProcessingId === editingItem.id ? <Loader2 className="animate-spin" /> : <ShieldCheck size={18} />}
                   {isProcessingId === editingItem.id ? 'Handshake API...' : 'Salvar & Autenticar Rede'}
                </button>
             </form>
          </div>
        </div>
