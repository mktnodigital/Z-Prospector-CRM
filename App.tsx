...
const ZLogo: React.FC<{ 
  ...
  if (status === 'error' || !src) return renderFallback();

  return (
    <div className={`relative flex items-center justify-center ${className}`}>
      {status === 'loading' && (
        <div className="absolute inset-0 flex items-center justify-center z-10 bg-slate-500/5 rounded-lg">
          <Loader2 className="animate-spin text-indigo-500/30" size={20} />
        </div>
      )}
      <img 
        ref={imgRef}
...
const App: React.FC = () => {
  ...
  // PERSISTÊNCIA DE BRANDING CORRIGIDA
  ...
  const handleBrandingChange = (newConfig: BrandingConfig) => {
    ...
    notify('Marca master atualizada com sucesso!');
  };
  ...
  // Efeito para injetar Favicon e Título
  useEffect(() => {
    const updateHeader = () => {
      // Favicon
      ...
      // Title
      document.title = `${branding.appName} - Master Authority`;
    };
        ...
        {showLoginModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-950/60 backdrop-blur-[30px] animate-in fade-in overflow-hidden transition-all duration-500">
             ...
             <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-3xl w-full max-w-xl rounded-[4rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] border border-slate-200 dark:border-white/10 relative overflow-hidden flex flex-col animate-in zoom-in-95 duration-500">
                <button onClick={() => setShowLoginModal(false)} className="absolute top-10 right-10 p-4 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 rounded-2xl transition-all text-slate-400 hover:text-rose-500 z-30 border border-slate-200 dark:border-white/5 shadow-sm"><X size={24} /></button>
                <div className="p-14 md:p-16 flex flex-col items-center text-center relative z-10">
                   <div className="mb-14 group transform transition-all duration-1000 hover:scale-105 relative">
                      ...
                      <ZLogo branding={branding} type="full" imgClassName="h-32 md:h-36 w-auto" className="drop-shadow-[0_10px_30px_rgba(79,70,229,0.2)] relative z-10" darkMode={isDarkMode} />
                   </div>
                   ...
                   <form onSubmit={handleLoginSubmit} className="w-full space-y-6 text-left">
                      <div className="space-y-3">
                         <div className="flex justify-between items-center px-4"><label className="text-[9px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-[0.3em]">Master Identity</label></div>
                         <div className="relative group">
                            <div className="absolute inset-y-0 left-0 pl-7 flex items-center pointer-events-none transition-colors group-focus-within:text-indigo-600 text-slate-300 dark:text-slate-600"><Mail size={22} /></div>
                            <input type="email" required placeholder="master@zprospector.com" value={loginForm.email} onChange={e => setLoginForm({...loginForm, email: e.target.value})} className="w-full pl-16 pr-8 py-6 bg-slate-50 dark:bg-slate-955/40 border border-slate-200 dark:border-white/10 rounded-[2rem] outline-none font-bold text-sm text-slate-900 dark:text-white focus:border-indigo-600/50 focus:bg-white dark:focus:bg-slate-950/60 focus:ring-8 focus:ring-indigo-600/5 transition-all placeholder:text-slate-300 dark:placeholder:text-slate-700 shadow-sm" />
                         </div>
                      </div>
                      <div className="space-y-3">
                         <div className="flex justify-between items-center px-4"><label className="text-[9px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-[0.3em]">Security Passphrase</label></div>
                         <div className="relative group">
                            <div className="absolute inset-y-0 left-0 pl-7 flex items-center pointer-events-none transition-colors group-focus-within:text-indigo-600 text-slate-300 dark:text-slate-600"><Lock size={22} /></div>
                            <input type={showPassword ? "text" : "password"} required placeholder="••••••••••••" value={loginForm.password} onChange={e => setLoginForm({...loginForm, password: e.target.value})} className="w-full pl-16 pr-16 py-6 bg-slate-50 dark:bg-slate-955/40 border border-slate-200 dark:border-white/10 rounded-[2rem] outline-none font-bold text-sm text-slate-900 dark:text-white focus:border-indigo-600/50 focus:bg-white dark:focus:bg-slate-950/60 focus:ring-8 focus:ring-indigo-600/5 transition-all placeholder:text-slate-300 dark:placeholder:text-slate-700 shadow-sm" />
                            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-7 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600 hover:text-indigo-600 transition-colors">{showPassword ? <EyeOff size={20} /> : <Eye size={20} />}</button>
                         </div>
                      </div>
                      <button type="submit" disabled={isLoggingIn} className="w-full py-7 bg-gradient-to-r from-indigo-600 via-indigo-500 to-indigo-700 text-white font-black rounded-[2rem] shadow-[0_20px_40px_-10px_rgba(79,70,229,0.4)] uppercase text-[12px] tracking-[0.4em] flex items-center justify-center gap-4 mt-6 hover:scale-[1.02] active:scale-95 transition-all border border-white/10 group overflow-hidden relative"><div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity"></div>{isLoggingIn ? <Loader2 className="animate-spin" size={22} /> : <Shield size={22} className="group-hover:rotate-12 transition-transform" />}{isLoggingIn ? 'Sincronizando Identidade...' : 'Unlock Console'}</button>
                   </form>
                   <div className="mt-14 flex flex-wrap items-center justify-center gap-10 opacity-40 grayscale group-hover:grayscale-0 transition-all"><div className="flex items-center gap-3"><ShieldCheck size={16} className="text-emerald-500" /><span className="text-[9px] font-black text-slate-900 dark:text-white uppercase tracking-[0.3em]">End-to-End Encrypted</span></div><div className="flex items-center gap-3"><Fingerprint size={16} className="text-indigo-500" /><span className="text-[9px] font-black text-slate-900 dark:text-white uppercase tracking-[0.3em]">Biometric ID Valid</span></div></div>
                </div>
             </div>
          </div>
        )}
  ...
  return (
    <div className={`fixed inset-0 flex h-full w-full overflow-hidden transition-colors duration-300 ${isDarkMode ? 'dark bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      {notification && (<div className="fixed top-10 left-1/2 -translate-x-1/2 z-[300] bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl animate-in slide-in-from-top-10 flex items-center gap-4"><Zap className="text-yellow-400 fill-current" size={16} />{notification}</div>)}
      <aside className={`${isSidebarOpen ? 'w-80' : 'w-24'} bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 transition-all duration-500 flex flex-col z-50 shadow-2xl relative h-full overflow-hidden`}>
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="absolute top-6 right-[-14px] w-7 h-7 bg-indigo-600 text-white rounded-full flex items-center justify-center shadow-lg hover:scale-110 active:scale-95 transition-all z-[60] border-2 border-white dark:border-slate-900">{isSidebarOpen ? <ChevronLeft size={16} /> : <ChevronRight size={16} />}</button>
        ...
        <nav className="flex-1 px-4 py-2 space-y-2 overflow-hidden hover:overflow-y-auto no-scrollbar">
          {modules.map((m) => (<button key={m.id} onClick={() => handleModuleChange(m.id as AppModule)} className={`w-full flex items-center ${isSidebarOpen ? 'gap-4 px-6 justify-start' : 'justify-center'} py-4 rounded-2xl transition-all group border-2 ${activeModule === m.id ? (m.id === 'admin' ? 'bg-orange-500 border-orange-500 shadow-orange-200/50' : 'bg-indigo-600 border-indigo-600 shadow-indigo-200/50') + ' text-white font-black shadow-xl' : `text-slate-500 dark:text-slate-400 border-transparent ${m.bg}`}`}><m.icon size={isSidebarOpen ? 22 : 30} className={`${activeModule === m.id ? 'text-white' : m.color} transition-all duration-300`} />{isSidebarOpen && <span className="text-[11px] font-black uppercase tracking-[0.2em] animate-in fade-in duration-500">{m.label}</span>}</button>))}
        </nav>
        <div className="p-6 border-t border-slate-100 dark:border-slate-800"><button onClick={handleLogout} className={`w-full flex items-center ${isSidebarOpen ? 'gap-4 px-6' : 'justify-center'} py-4 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-2xl transition-all`}><LogOut size={isSidebarOpen ? 18 : 24} /> {isSidebarOpen && <span className="text-[10px] font-black uppercase">Sair</span>}</button></div>
      </aside>
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-24 bg-white/60 dark:bg-slate-955/60 backdrop-blur-2xl border-b border-slate-100 dark:border-slate-800 flex items-center justify-between px-12 z-40">
          <div className="flex items-center gap-8">
            <div className="hidden lg:flex items-center gap-4 px-8 py-3 bg-emerald-600/10 text-emerald-600 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-emerald-600/20"><ShieldCheck size={16} className="text-yellow-500" /> Licença Ativa (SaaS Master)</div>
            <button onClick={toggleTheme} className="p-3 bg-slate-100 dark:bg-slate-800 rounded-xl text-slate-500 dark:text-yellow-400 transition-all hover:scale-110">{isDarkMode ? <Sun size={20} /> : <Moon size={20} />}</button>
          </div>
          <div className="flex items-center gap-4 cursor-pointer group" onClick={() => handleModuleChange('profile')}>
             <div className="text-right hidden sm:block"><div className="flex items-center justify-end gap-2"><p className="text-xs font-black uppercase tracking-widest">{currentUser.name}</p><div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div></div><p className="text-[9px] font-black text-indigo-500 uppercase tracking-widest">Master Authority • Online</p></div>
             <div className="w-14 h-14 rounded-2xl bg-indigo-600 text-white flex items-center justify-center font-black shadow-2xl border-4 border-white dark:border-slate-800 transition-all group-hover:scale-105 active:scale-95 group-hover:border-indigo-500 relative overflow-hidden">
                {currentUser.avatar ? (<img src={currentUser.avatar} className="w-full h-full object-cover" alt="User" />) : (<span className="text-xl font-black">{currentUser.name.charAt(0)}</span>)}
        ...
        <div className="flex-1 overflow-auto custom-scrollbar bg-white dark:bg-slate-955">
          ...
          {activeModule === 'scheduling' && (<ScheduleManager appointments={appointments} onAddAppointment={(a) => setAppointments([...appointments, a])} onUpdateAppointment={(a) => setAppointments(prev => prev.map(item => item.id === a.id ? a : item))} onDeleteAppointment={(id) => setAppointments(prev => prev.filter(item => item.id !== id))} />)}
