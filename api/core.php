...
// -------------------------------------------------------------------------
// SaaS SECURITY LAYER: DYNAMIC TENANT RESOLUTION
// -------------------------------------------------------------------------
// Obtém o Tenant ID do Header da requisição (Seguro) ou da URL (Fallback)
// Se nenhum for fornecido, assume 1 (Master) para compatibilidade regressiva.
$tenant_id = $_SERVER['HTTP_X_TENANT_ID'] ?? $_GET['tenant_id'] ?? 1;

// Sanitização básica para evitar SQL Injection via ID
